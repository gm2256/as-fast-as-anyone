...
def run(cfg: DictConfig) -> None:
    ...
    if env_config["mixture_datasets"]:
        paths_weights = [(m["path"], m["weight"]) for m in env_config["mixture_datasets"]]
        print("-> Mixing training data pools:")
        for path, weight in paths_weights:
            print(f"   {weight:.3f}  {path}")
        data_generator = simulator.make_mixture_data_generator(
