...
def train(
    ...
    Args:
        ...
        pretrained_params_path: Path to a params-only checkpoint (as written by
            ...
            state and step counters start fresh. Meant for transferring a
            policy across runs/datasets (e.g. jeju -> hanam). Ignored if a
            full checkpoint is resumed (see `resume`).
        resume: If True (default) and `checkpoint_logdir/train_state_latest.pkl`
            exists, restore the *full* training state (params, both optimizer
            states, rl_gradient_steps, env_steps) from it and continue this
            same run from where it left off - `total_timesteps` is treated as
            an absolute target, so remaining steps = total_timesteps -
            env_steps already done. This is what makes it safe to kill and
            re-launch the same `name_run` (e.g. after a Colab disconnect)
            without losing progress. Set False to force a fresh run even if a
            checkpoint is present. The replay buffer itself is not persisted;
            it is refilled via the normal prefill step.
