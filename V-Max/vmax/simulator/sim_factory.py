...
def make_data_generator(
    ...
    Args:
        ...
        max_num_rg_points: Max number of roadgraph points to parse per record.
            Must match the number of points actually stored in the dataset's
            tfrecords (see scripts/make_91f.py's roadgraph filtering) or
...
def make_mixture_data_generator(
    paths_weights: list[tuple[str, float]],
    max_num_objects: int = 128,
    max_num_rg_points: int = 30000,
    include_sdc_paths: bool = True,
    seed: int | None = None,
    ...
    """Yield batches drawn from several datasets at fixed, independent odds.

    Each `(path, weight)` pool (e.g. scripts/split_hard_easy_pools.py's
    per-site hard/easy dirs) keeps its full file set - nothing is dropped.
    Only how OFTEN a given pool is drawn from is controlled, so biasing
    training toward a "hard" or a given site's pool never shrinks how much
    data the policy sees; it only reweights the sampling odds per step.

    Args:
        paths_weights: (dataset path, relative weight) pairs; weights need
            not sum to 1, they are normalized.
        max_num_objects: Maximum number of objects.
        max_num_rg_points: Max roadgraph points to parse per record (must
            match every pool's actual stored point count).
        include_sdc_paths: Whether to include SDC paths.
        seed: Seed for both the per-pool shuffling and the pool-choice draws.
        batch_dims: Batch dimensions.
        distributed: Whether to distribute the dataset.
        repeat: Number of repetitions per pool (None for infinite).

    Returns:
        A generator yielding simulator states.

    """
    paths = [p for p, _ in paths_weights]
    weights = np.array([w for _, w in paths_weights], dtype=np.float64)
