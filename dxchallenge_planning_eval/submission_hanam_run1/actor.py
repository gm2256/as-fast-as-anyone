"""V-Max SAC baseline (run hanam_run1) as a challenge submission.
...
# Lockstep batch width this submission is evaluated with (see README 제출물 형식).
BATCH_SIZE = 64

# observation_config of the training run (runs/hanam_run1/.hydra/config.yaml).
OBSERVATION_CONFIG = {
...
class HanamRun1Planner(actor_core.WaymaxActorCore):
    """SAC policy (LQ encoder) trained with the V-Max baseline (jeju_run1 warm-start -> hanam)."""
...
def create_actor(submission_dir: str) -> actor_core.WaymaxActorCore:
    ...
    return HanamRun1Planner(params)
