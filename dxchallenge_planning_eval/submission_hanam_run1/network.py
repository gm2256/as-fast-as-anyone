...
def build_policy_network(unflatten_fn: Callable) -> PolicyNetwork:
    ...
    run hanam_run1 (runs/hanam_run1/.hydra/config.yaml): LQ encoder
    (network.encoder) + MLP head (algorithm.network.policy) + Dense(4) for
    the gaussian (loc, scale) params.
    """
    encoder = LQEncoder(
    ...
    fully_connected = MLP(layer_sizes=(256, 256), activation=nn.relu)
