...
per episode, over the full horizon:
    progress_ratio  -> value at the final step
...
def episode_scores(step_metrics: dict[str, np.ndarray]) -> dict:
    """Aggregate per-step metric arrays of one episode into episode-level values.

    Args:
        step_metrics: Mapping metric name -> array of per-step values over the
            episode.
